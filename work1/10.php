	...
	if(isset ($_POST['send'])){
		...
		if($isok==true)  {
	                      ...
	                      if(   $conn->commit() === TRUE ) { echo "<h4>"."تم ادخال البيانات بنجاح"."</h4>";
			                                          ...
			                                          $req="select * from emp_vw where id_num=$id_num";
			                                       	$query=mysqli_query($conn,$req);
															
															
								while($fetch=mysqli_fetch_object($query)) {echo  $fetch->id_num." ";
...
الدائرة : <input class="form-control" type="number" name="dept_id" required/>
<br>
رقم هوية الموظف : <input class="form-control" type="number" name="id_num" required/>
<br>
اسم الموظف : <input class="form-control" type="text" name="emp_name" required/>
<br>
ملاحظات : <input class="form-control" type="text" name="emp_note" required/>
<br>	
الراتب الشهري : <input class="form-control" type="number" name="sal" required/>
<br>
	</div>
